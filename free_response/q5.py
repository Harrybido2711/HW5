...
def q5(env, title="Plot"):
    ...
    plt.legend()
    plt.savefig(f"free_response/5a_{title}.png")
    plt.clf()
...
def main():
    ...
    with warnings.catch_warnings():
        ...
        q5(slots, title="SlotMachines_Comparision")
        print(".", end="", flush=True)
      
        frozen_lake = gymnasium.make('FrozenLake-v1', map_name='4x4', is_slippery=False)
        q5(frozen_lake, title="FrozenLake_Comparison")
        print(".", end="", flush=True)

        frozen_lake = gymnasium.make('FrozenLake-v1', map_name='4x4', is_slippery=True)
        q5(frozen_lake, title="SlipperyFrozenLake_Comparison")
        print()
        print("Done! See plots in `free_response/5a*.png`")
