...
from src import load_data, KNearestNeighbor
from src.random import rng, uniform, normal
...
def test_knn_k_is_1():
    goals = {
      'blobs.csv': {1.0: 1.0, 0.8: 0.4},
      'circles.csv': {1.0: 1.0, 0.5: 0.7, 0.1: 0.7},
      'ivy-league.csv': {1.0: 0.9, 0.8: 0.5},
...
def test_knn_k_is_big():
    ...
    try:
        ...
        assert False, f"If K > N, just set K = N instead of crashing: '{e}'"
...
def generate_regression_data(degree, amount):
    from sklearn.preprocessing import PolynomialFeatures
    from sklearn.linear_model import LinearRegression

    model = LinearRegression()
    model.intercept_ = 0
    model.coef_ = uniform(-10, 10, (1, degree+1))
    transform = PolynomialFeatures(degree)

    x = uniform(-1.0, 1.0, (amount, 1))
    y = model.predict(transform.fit_transform(x))
    return x, y
